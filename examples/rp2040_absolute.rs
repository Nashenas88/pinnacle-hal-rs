...
use pinnacle_touchpad::{AbsoluteData, PinnacleTouchpad, PinnacleTouchpadBuilder};
...
static mut PINNACLE: Option<(
    Delay,
    TouchpadDrPin,
    PinnacleTouchpad<TouchpadSpi, SpiCs, AbsoluteData>,
)> = None;
...
fn IO_IRQ_BANK0() {
    let Some((delay, touchpad_irq, pinnacle)) = (unsafe { PINNACLE.as_mut() }) else {
        defmt::info!("no structs");
        return;
    };

    if !touchpad_irq.interrupt_status(Interrupt::EdgeHigh) {
        defmt::info!("not right interrupt");
        return;
    }

    let data = pinnacle.read_absolute().unwrap();
